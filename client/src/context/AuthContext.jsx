...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://final-project-za5c.onrender.com/api'}/auth/login`, {
  ...
  const register = async (name, email, password, role, phone, cacUrl, hostelDocUrl, whatsapp, telegram) => {
    const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://final-project-za5c.onrender.com/api'}/auth/register`, {
